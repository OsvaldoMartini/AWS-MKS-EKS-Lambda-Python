...
async function extractFullXPath(url) {
  ...
  try {
    ...
    const xpathExpressions = new Set();

    // Find all elements on the page
    const elements = await driver.findElements(By.xpath("//*"));

    // Generate full XPath for each element and add it to the Set
    for (const element of elements) {
      const fullXPath = await getFullXPath(driver, element);
      xpathExpressions.add(fullXPath);
    }

    return Array.from(xpathExpressions);
  } catch (error) {
...
async function main() {
  ...
  const xpathFilename = "xpath-javascript-found-8.txt";
